...
from testbed.models.card._card_repo._diffusion_utils import *
from testbed.models.card._card_repo._ema import EMA
from testbed.models.card._card_repo._model import *
...
class Diffusion:
    def __init__(self, args, config, dataset: Dataset, device=None):

        # --------------------------------
        # Testbed changes
        # --------------------------------
        self._testbed_dataset = dataset
        # --------------------------------
        ...
        if config.diffusion.conditioning_signal == "OLS":
            ...
            _, dataset = self._get_dataset(args, config, test_set=False)
    ...
    def _get_dataset(self, args, config, test_set=False, validation=False):
        """
        Helper function that simply intercepts previous get_dataset function
    ...
    def compute_unnorm_y(self, cur_y, testing):
        if testing:
            y_mean = (
                cur_y.cpu().reshape(-1, self.config.testing.n_z_samples).mean(1).reshape(-1, 1)
            )
        else:
            y_mean = cur_y.cpu()
    ...
    def make_subplot_at_timestep_t(
        ...
        #     fontsize=20)
        kl_unnorm_str = f"Unnormed RMSE: {kl_unnorm:.2f}"
        if prior:
            axs[ax_idx].set_title(f"$p({y}_{prior})$", fontsize=23)
            axs[ax_idx].set_title(f"$p({y}_{prior})$\n" + kl_unnorm_str, fontsize=23)
            axs[ax_idx].legend()
        else:
            axs[ax_idx].set_title("$p(y_{" + str(cur_t) + "})", fontsize=23)
            axs[ax_idx].set_title("$p(y_{" + str(cur_t) + "})\n" + kl_unnorm_str, fontsize=23)
    ...
    def train(self):
        ...
        test_set_object, test_set = self._get_dataset(args, config, test_set=True)
        ...
        dataset_object, dataset = self._get_dataset(args, config, test_set=False)
        ...
        if (
            ...
            val_set_object, val_set = self._get_dataset(
                        ...
                        if (
                            ...
                            with torch.no_grad():
                                ...
                                fig.savefig(
                                    os.path.join(
                                        args.im_path,
                                        f"samples_T{self.num_timesteps}_{step}.png",
                                    )
                                )
                            plt.close("all")
                            ...
                            with torch.no_grad():
                                ...
                                if epoch == start_epoch:
                                    ...
                                    axs[0].set_title("$q(y_{" + str(1) + "})$", fontsize=23)
                                    y_q_seq = []
                                    for j in range(1, self.num_figs):
                                        cur_t = j * self.vis_step
                                        cur_y = (
                                        ...
                                        axs[j].scatter(
                                            x_batch.detach().cpu(), cur_y, s=10, c="tab:red"
                                        )

                                        axs[j].set_title(
                                            "$q(y_{" + str(cur_t) + "})$", fontsize=23
                                        )
                                    ...
                                    axs[self.num_figs].set_title(
                                        "$q(y_{" + str(self.num_timesteps) + "})$", fontsize=23
                                    )
                                    ax_list = [axs[j] for j in range(self.num_figs + 1)]
                                    ax_list[0].get_shared_x_axes().join(ax_list[0], *ax_list)
                                    ax_list[0].get_shared_y_axes().join(ax_list[0], *ax_list)
                                ...
                                axs[0].set_title("$p(y_1)$", fontsize=23)
                                # axs[0].set_xlabel(
                                ...
                                axs[self.num_figs].set_title("$p({z}_{prior})$", fontsize=23)
                                # kl = kld(y_T, cur_y)
                                # kl_y0 = kld(y_batch.detach().cpu(), cur_y)
    ...
    def sample(self):
        ...
        if self.config.data.dataset == "swiss_roll":
            ...
            xy_0 = Gaussians().sample(
                self.config.sampling.sampling_size - self.config.sampling.sampling_size % 8,
                mode=8,
            )
        elif self.config.data.dataset == "25gaussians":
            xy_0 = Gaussians().sample(
                self.config.sampling.sampling_size - self.config.sampling.sampling_size % 25,
                mode=25,
            )
        ...
        with torch.no_grad():
            ...
            for i in range(self.num_figs - 1):
                ...
                axs[i].set_title("$q({y}_{" + str(i * self.vis_step) + "})$", fontsize=23)
            ...
            axs[self.num_figs - 1].set_title(
                "$q(y_{" + str(self.num_timesteps - 1) + "})$", fontsize=23
            )
            fig.savefig(
                os.path.join(
                    self.args.im_path, f"diffusion_samples_T{self.num_timesteps}_{ckpt_id}.png"
                )
            )
            ...
            for i in range(self.num_figs - 1):
                ...
                axs[0, i].set_title("$q(y_{" + str(i * self.vis_step) + "})$", fontsize=23)
                axs[1, i].scatter(x_batch, cur_y, s=10)
                axs[1, i].set_title(
                    "$p_{\\theta}(y_{" + str(i * self.vis_step) + "})$", fontsize=23
                )
                axs[1, i].set_xlabel(
                    f"KL($q(y_t)||p(y_t)$)={kl:.2f}\nKL($q(y_0)||p(y_t)$)={kl_y0:.2f}",
                    fontsize=20,
                )
            ...
            axs[1, self.num_figs - 1].set_title(
                "$p_{\\theta}(y_{" + str(self.num_timesteps) + "})$", fontsize=23
            )
            axs[1, self.num_figs - 1].set_xlabel(
                f"KL($q(y_t)||p(y_t)$)={kl:.2f}\nKL($q(y_0)||p(y_t)$)={kl_y0:.2f}", fontsize=20
            )
            axs[1, self.num_figs - 1].tick_params(axis="x", labelsize=20)
            axs[1, self.num_figs - 1].tick_params(axis="y", labelsize=20)
            fig.savefig(
                os.path.join(
                    self.args.im_path, f"generated_samples_T{self.num_timesteps}_{ckpt_id}.png"
                )
            )
            ...
            axs[1, self.num_figs - 1].set_title(
                "$p_{theta}(y_{" + str(self.num_timesteps) + "})$", fontsize=23
            )
            ...
            fig.savefig(
                os.path.join(
                    self.args.im_path,
                    f"generated_distribution_T{self.num_timesteps}_{ckpt_id}.png",
                )
            )
        plt.close("all")
    ...
    def test(self):
        ...
        def compute_prediction_SE(
            ...
            if dataset_object.normalize_y:
                ...
                y_pred_mean = dataset_object.scaler_y.inverse_transform(y_pred_mean).astype(
                    np.float32
                )
            if return_pred_mean:
                return y_pred_mean
        ...
        dataset_object, dataset = self._get_dataset(args, config, test_set=True)
        test_loader = data.DataLoader(
            dataset,
            batch_size=config.testing.batch_size,
            num_workers=config.data.num_workers,
        ...
        if config.diffusion.conditioning_signal == "NN":
            aux_states = torch.load(
                os.path.join(log_path, "aux_ckpt.pth"), map_location=self.device
            )
            self.cond_pred_model.load_state_dict(aux_states[0], strict=True)
            self.cond_pred_model.eval()
        ...
        with torch.no_grad():
            ...
            for step, xy_batch in enumerate(test_loader):
                ...
                if (
                    ...
                    fig.savefig(
                        os.path.join(args.im_path, f"samples_T{self.num_timesteps}_{step}.png")
                    )
                    plt.close("all")
        ...
        if config.testing.compute_metric_all_steps:
            ...
            y_qice_all_steps_list.append(qice_coverage_ratio)
            logging.info(f"\nWe generated {config.testing.n_z_samples} y's given each x.")
            logging.info(
                (
            ...
            y_nll_all_steps_list.append(y_nll)
            logging.info(f"\nNegative Log-Likelihood on test set is {y_nll:.8f}.")
        ...
        if config.testing.make_plot:
            ...
            if dataset_object.normalize_y:
                all_true_y = dataset_object.scaler_y.inverse_transform(all_true_y).astype(
                    np.float32
                )
            ...
            fig_3.savefig(
                os.path.join(args.im_path, "quantile_interval_coverage_true_vs_optimal.png"),
                dpi=1200,
            )
