...
parser.add_argument(
    ...
    type=int,
    default=20,
    help="total number of splits for a specific regression task",
)
parser.add_argument(
...
if __name__ == "__main__":
    if args.run_all:
        ...
        if args.run_all and args.test:
            ...
            y_nll_all_splits_list = [
                metric_list[nll_idx] for metric_list in y_nll_all_splits_all_steps_list
            ]
            ...
            print(
                f"QICE mean: {np.mean(y_qice_all_splits_list)} QICE std: {np.std(y_qice_all_splits_list)}"
            )
            print(
                f"PICP mean: {np.mean(y_picp_all_splits_list)} PICP std: {np.std(y_picp_all_splits_list)}"
            )
            print(
                f"NLL mean: {np.mean(y_nll_all_splits_list)} NLL std: {np.std(y_nll_all_splits_list)}"
            )
            ...
            y_rmse_mean_all_splits_list = [
                np.mean(y_rmse_all_splits_all_steps_array[:, idx])
                for idx in range(n_timesteps + 1)
            ]
            y_qice_mean_all_splits_list = [
                np.mean(y_qice_all_splits_all_steps_array[:, idx])
                for idx in range(n_timesteps + 1)
            ]
            y_picp_mean_all_splits_list = [
                np.mean(y_picp_all_splits_all_steps_array[:, idx])
                for idx in range(n_timesteps + 1)
            ]
            y_nll_mean_all_splits_list = [
                np.mean(y_nll_all_splits_all_steps_array[:, idx])
                for idx in range(n_timesteps + 1)
            ]
            n_metric = 4
