...
class SwissRoll:
    ...
    def sample(self, n, noise=0.5):
        ...
        return torch.from_numpy(
            make_swiss_roll(n_samples=n, noise=noise)[0][:, [0, 2]].astype("float32") / 5.0
        )
...
class UCI_Dataset:
    def __init__(self, config, split, validation=False):
        ...
        index_train = np.loadtxt(
            _get_index_train_test_path(_DATA_DIRECTORY_PATH, split, train=True)
        )
        index_test = np.loadtxt(
            _get_index_train_test_path(_DATA_DIRECTORY_PATH, split, train=False)
        )
...
class LinearDatasetWithOneX(DatasetWithOneX):
    def __init__(
        self, a, b, n_samples, seed, x_dict, noise_dict, normalize_x=False, normalize_y=False
    ):
        self.a = a
        self.b = b
...
class QuadraticDatasetWithOneX(DatasetWithOneX):
    ...
    def create_y_from_one_x(self, noise_dict):
        super().create_y_from_one_x(noise_dict)
        y = (
            self.a * self.x_samples.pow(2)
            + self.b * self.x_samples
            + self.c
            + self.eps_samples
        )
        return y
...
class CircleDatasetWithOneX(DatasetWithOneX):
    def __init__(
        self, r, n_samples, seed, x_dict, noise_dict, normalize_x=False, normalize_y=False
    ):
        self.r = r
    ...
    def create_y_from_one_x(self, noise_dict):
        ...
        self.x_samples, y = r_samples * torch.cos(self.theta), r_samples * torch.sin(
            self.theta
        )
...
class LinearDatasetWithStdIncreaseWithX(LinearDatasetWithOneX):
    ...
    def create_y_from_one_x(self, noise_dict):
        ...
        noise_dict.update(
            noise_type="norm", scale=noise_dict.get("scale", 1.0) * self.x_samples.abs()
        )
...
class SubspaceInferenceDatasetWithOneX(DatasetWithOneX):
    ...
    def create_y_from_one_x(self, noise_dict):
        ...
        x_ranges_num_samples = [
            int(x / x_ranges_total_len * self.n_samples) for x in x_ranges_len
        ]
            ...
            self.x_samples = torch.cat(
                [
                    ...
                    torch.distributions.uniform.Uniform(
                        low=self.x_ranges[idx][0], high=self.x_ranges[idx][1]
                    ).sample((x_ranges_num_samples[idx], 1)),
                ],
                axis=0,
            )
        self.dim_x = self.x_samples.shape[1]
        y = (
            utils.SubspaceInferenceDatasetNet().forward(
                x=torch.cat([self.x_samples, self.x_samples**2], axis=1)
            )
            + self.eps_samples
        )
        return y
...
class CTToyDataset(DatasetWithOneX):
    ...
    def load_data(self, name):
        ...
        if name == "swiss_roll":
            ...
            temp = []
            for i in range(N):
                point = np.random.randn(2) * 0.1  # .02
                center = centers[np.random.choice(np.arange(len(centers)))]
                point[0] += center[0]
                point[1] += center[1]
                temp.append(point)
