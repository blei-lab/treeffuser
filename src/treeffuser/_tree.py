...
def _integrate_divergence_over_time(
    ...
    integral_left = (
        _integrate_divergence_over_time(node["left_child"], y, x, t_min_left, t_max_left)
        if integrate_left
        else 0
    )

    integral_right = (
        _integrate_divergence_over_time(node["right_child"], y, x, t_min_right, t_max_right)
        if integrate_right
        else 0
    )
