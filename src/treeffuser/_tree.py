...
def _compute_score_divergence_numerical(score_function, y, x, t, eps=10 ** (-5)):
    div = (score_function(y + eps, x, t) - score_function(y, x, t)) / eps
...
def _compute_score_divergence(
    ...
    div = 0
    for tree in forest:
        div += _compute_divergence_tree(tree["tree_structure"], y, x, t)
    return learning_rate * div


def _compute_divergence_tree(
    node: dict,
    y: Float[np.ndarray, "y_dim"],
    x: Float[np.ndarray, "x_dim"],
    t: float,
...
def _compute_divergence_leaf(node: dict, y_dim: int):
    div = 0
